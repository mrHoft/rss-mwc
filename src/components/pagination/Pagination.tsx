...
const Pagination: React.FC<PaginationProps> = ({ page, total, pageSize = 10, onChange }) => {
  ...
  const btns = Array.from({ length: Math.min(len, 5) }, (_, i) => {
    const n = first + i;
    return (
      <button className={styles.pagination__btn} key={n} onClick={() => onChange(n)} disabled={page === n}>
        {n + 1}
      </button>
    );
  });
  ...
  return (
    <div className={styles.pagination} onClick={(e) => e.stopPropagation()} data-testid="pagination">
      ...
      {page >= 3 && len > 5 && (
        <>
          <button className={styles.pagination__btn} onClick={() => onChange(0)} disabled={page === 0}>
            1
          </button>
          {first > 1 && <span>•••</span>}
        </>
      ...
      {page + 3 < len && len > 5 && (
        <>
          ...
          <button className={styles.pagination__btn} onClick={() => onChange(len - 1)} disabled={page === len - 1}>
            {len}
          </button>
        </>
      )}
      <button className={styles.pagination__slide} onClick={rightClickHandler} disabled={page === len - 1}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
